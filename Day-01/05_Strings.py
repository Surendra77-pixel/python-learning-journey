...
# The strings as array i also call these as the index which means the unicode character 
...
# by using the for loop the string can be looped through each character in the string one by one. Here is an example:
...
#we can use the in keyword to check if a substring exists within a string. they are in , not , if , if not  Here is an example:
...
# we can use slicing to extract a portion of a string. The syntax for slicing is string[start:end], where start is the index of the first character to include and end is the index of the first character to exclude. Here is an example:
...
# we can also use negative indices to slice a string. Negative indices count from the end of the string, with -1 being the last character. Here is an example:
